...
uniform float light_radius_globe;
...
void main()
{
    ...
    vec3 light_floor = vec3(0.f, 0.f, 0.f);
    vec3 light_spot = vec3(0.f, 0.f, 0.f);
    vec3 light_globe = vec3(0);
    ...
    if (is_floor == 1) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            light_floor +=  pointLightColor[i] * (attenuation_factor * PointLightContribution(pointLightPos[i]));
        }
    }
    ...
    if (is_globe == 1) {
        ...
        if (d < light_radius_globe) {
            attenuation_factor = pow(light_radius_globe - d, 2);
        }

        light_globe +=  0.5 * color_globe * (attenuation_factor * PointLightContribution(disco_ball_position));
        
    }

     out_color = vec4((light_floor + light_spot + light_globe), 1); 
